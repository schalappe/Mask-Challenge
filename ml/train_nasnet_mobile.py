...
# data generator
train_generator = DatasetGenerator(FEATURES_PATH + "train_set.hdf5", BS, aug=aug, preprocessors=[pp, iap])
train_set = tf.data.Dataset.from_generator(
    train_generator.generator, output_types=(tf.float64, tf.uint8), output_shapes=OUTPUT_SHAPE
).prefetch(tf.data.AUTOTUNE)

test_generator = DatasetGenerator(FEATURES_PATH + "test_set.hdf5", BS, preprocessors=[sp, iap])
test_set = tf.data.Dataset.from_generator(
    test_generator.generator, output_types=(tf.float64, tf.uint8), output_shapes=OUTPUT_SHAPE
).prefetch(tf.data.AUTOTUNE)

# construct our model
print("[INFO]: Create model")
head = tf.keras.applications.NASNetMobile(input_shape=(224, 224, 3), include_top=False, weights="imagenet", classes=2)
...
# callbacks
callbacks = [tf.keras.callbacks.TensorBoard(log_dir=LOGS_PATH + "nasnet/", profile_batch=0)]
...
# callbacks
callbacks = [
    tf.keras.callbacks.TensorBoard(log_dir=LOGS_PATH + "nasnet-fine/", profile_batch=0),
    tf.keras.callbacks.ModelCheckpoint(
        MODEL_PATH + "best_nasnet.h5",
        monitor="val_loss",
        mode="min",
        verbose=1,
        save_best_only=True,
